...
$$
d(a,b)=\left|\mathrm{wrap}_{[-180,180)}(b-a)\right|
$$
...
$$
P_{\mathrm{avail}}=\min\left(\frac{1}{2}\eta\rho A C_p v^3 \cos^3(e_{\mathrm{yaw}}),P_{\mathrm{rated}}\right)
$$
...
- In `Komega^2` mode: $P_{\mathrm{expected}}=P_{\mathrm{avail}}$.
- In down-regulation mode: $P_{\mathrm{expected}}=\min(P_{\mathrm{setpoint}},P_{\mathrm{avail}})$.
- When disabled or shutdown: $P_{\mathrm{expected}}=0$.

The alarm triggers if average measured power stays too far away from average expected power for $5\mathrm{s}$:

$$
\left|\overline{P}_{\mathrm{measured}}-\overline{P}_{\mathrm{expected}}\right|>\max\left(500\mathrm{kW},0.05\overline{P}_{\mathrm{expected}}\right)
$$
...
$$
\theta_{\mathrm{pred}}(t+\Delta t)=\mathrm{norm}\left(\theta(t)+\mathrm{clamp}\left(\mathrm{wrap}(\theta_{\mathrm{set}}-\theta(t)),-r_{\mathrm{yaw}}\Delta t,r_{\mathrm{yaw}}\Delta t\right)\right)
$$
...
$$
P_{\mathrm{expected}}=\eta \cdot \mathrm{rpm}\cdot\frac{2\pi}{60}\cdot T_{\mathrm{gen}}\cdot G
$$
...
$$
\left|P_{\mathrm{measured}}-P_{\mathrm{expected}}\right|>400\mathrm{kW}
$$
...
$$
\left|\sum_i \overline{P}_{\mathrm{received},i}-\overline{P}_{\mathrm{total,measured}}\right|>10\mathrm{MW}
$$
...
$$
d(\mathrm{wd}_{\mathrm{turbine}},\mathrm{wd}_{\mathrm{global}})>25^\circ
$$
...
$$
\left|v_{\mathrm{new}}-\mathrm{median}(v_{\mathrm{previous}})\right|>3\mathrm{m/s}
$$

and:

$$
\max(v_{\mathrm{history}})-\min(v_{\mathrm{history}})>4\mathrm{m/s}
$$

For wind direction:

$$
d\left(\mathrm{wd}_{\mathrm{new}},\mathrm{circularMean}(\mathrm{wd}_{\mathrm{previous}})\right)>25^\circ
$$

and:

$$
\mathrm{angularSpread}(\mathrm{wd}_{\mathrm{history}}) > 40^\circ
$$
...
- The observed range is exactly zero, where $\mathrm{range}(x)=\max(x)-\min(x)$.
...
$$
\mathrm{rpm}_{\mathrm{expected}}=\mathrm{clamp}\left(\frac{\lambda_{\mathrm{opt}}v_{\mathrm{eff}}}{R}\frac{60}{2\pi},\mathrm{rpm}_{\min},\mathrm{rpm}_{\mathrm{rated}}\right)
$$

The corresponding generator torque is:

$$
T_{\mathrm{expected}}=\frac{P_{\mathrm{expected}}}{\eta\omega_{\mathrm{rotor}}G}
$$
...
$$
|T_{\mathrm{measured}}|<\max\left(200\mathrm{Nm},0.15T_{\mathrm{aero}}\right)
$$
...
This detector is the first line of defence against impossible values. Fresh measurements, within $3\mathrm{s}$, are checked against broad engineering limits:

- Wind speed: $0 \le v \le 1.5v_{\mathrm{cutout}}$.
...
- Power: $0 \le P \le 1.25P_{\mathrm{rated}}$.
- Generator torque: $0 \le T \le 1.50T_{\max}$.
...
$$
r_P=\frac{P_{\mathrm{measured}}}{P_{\mathrm{expected}}},\qquad r_{\mathrm{rpm}}=\frac{\mathrm{rpm}_{\mathrm{measured}}}{\mathrm{rpm}_{\mathrm{expected}}}
$$

Then it computes the fleet median and median absolute deviation:

$$
\mathrm{MAD}(r)=\mathrm{median}\left(\left|r_i-\mathrm{median}(r)\right|\right)
$$

A turbine is suspicious when either normalized value is far away from the fleet:

$$
\left|r_P-\mathrm{median}(r_P)\right|>\max\left(0.30,4\mathrm{MAD}(r_P)\right)
$$

or:

$$
\left|r_{\mathrm{rpm}}-\mathrm{median}(r_{\mathrm{rpm}})\right|>\max\left(0.25,4\mathrm{MAD}(r_{\mathrm{rpm}})\right)
$$
